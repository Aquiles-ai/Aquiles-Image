...
class LTX_2_Pipeline:
    ...
    def start(self):
        ...
        self.pipeline = TI2VidTwoStagesPipeline(checkpoint_path=f"{data_dir}/ltx-2-19b-dev.safetensors",
        gemma_root=f"{data_dir}/gemma", loras=[], 
        distilled_lora=[LoraPathStrengthAndSDOps(path=f"{data_dir}/ltx-2-19b-distilled-lora-384.safetensors", 
                        strength=0.6, sd_ops=LTXV_LORA_COMFY_RENAMING_MAP)], 
    ...
    def verify_model(self):
        ...
        if(file_exists(f"{model_path}/gemma/model-00004-of-00005.safetensors") and file_exists(f"{model_path}/ltx-2-19b-dev.safetensors") and file_exists(f"{model_path}/ltx-2-spatial-upscaler-x2-1.0.safetensors") and file_exists(f"{model_path}/ltx-2-19b-distilled-lora-384.safetensors")):
            pass
        else:
            download_ltx_2()
