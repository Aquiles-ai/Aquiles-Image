    ...
    def initialize_pipeline(self):
        ...
        if self.model in self.stablediff3:
            ...
            if self.model == 'diffusers/FLUX.2-dev-bnb-4bit':
                self.pipeline = PipelineFlux2(self.model, True, self.device_map_flux2, self.dist_inf, load_lora=self.load_lora, conf_lora=self.conf_lora)
            else:
            ...
            if self.auto_type is not None:
                if self.auto_type == "t2i":
                    ...
                    self.pipeline = AutoPipelineDiffusers(self.model, self.dist_inf, load_lora=self.load_lora, conf_lora=self.conf_lora)
                elif self.auto_type == "i2i":
                    logger_p.info(f"Loading model '{self.model}' with 'AutoPipelineI2IDiffusers'")
                    self.pipeline = AutoPipelineI2IDiffusers(self.model, self.dist_inf, load_lora=self.load_lora, conf_lora=self.conf_lora)
            else:
